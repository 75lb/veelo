...
HandbrakeCLI.prototype.spawn = function(settings) {
	...
	var output = "",
		self = this,
        args = settings.handbrakeArgs;
	
    if (Object.keys(args).length < 2){
        throw new Error("you must pass at least 2 args: -i and -o");
    }
    
	args = flattenArgsHash(args);
	var	handle = cp.spawn(_HandbrakeCLIPath, args);
	...
	handle.on("exit", function(code, signal){
		if (signal == "SIGTERM"){
			...
			util.log("encode failed, not a video file: " + args[args.indexOf("-i") + 1].fileName);
			self.emit(e.fail);
