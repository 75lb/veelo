...
Handbraker.prototype.start = function(){
	var self = this,
		...
		inputFiles = _config.passedIn.files;
	...
	if (options.handbraker.h || 
		...
		(inputFiles.length == 0 && _.intersection(options.handbrake, ["--preset-list", "--scan", "--title"]).length == 0)){
		getUsage(options.handbraker.hbhelp, function(usage){
			self.emit("message", usage);
		});

	// user passed --preset-list, --title or --scan
	} else if (inputFiles.length == 0 && _.intersection(options.handbrake, ["--preset-list"]).length > 0) {
