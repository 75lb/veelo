...
exports.run = function(options, onComplete){
    var config = options instanceof Config
        ? options
        : configMaster.get("handbrake", options);
    
    if (onComplete){
        var cmd = util.format('"%s" %s', _HandbrakeCLIPath, general.flattenArgsHash(config.toJSON()).join(" "));
        ...
        var output = "",
            ...
            handle = cp.spawn(_HandbrakeCLIPath, flattenArgsHash(config.toJSON()));
...
configMaster.add("handbrake", exports.run.config);
