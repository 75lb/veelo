...
Job.prototype.init = function(){
	...
	} else if (fileIgnored(file, _config.options.handbraker.ignoreList)){
		this.valid = false;
	} else{
		this.valid = true;
		this.emit("valid");
...
Job.prototype.process = function(){
	...
	this.emit("processing", this.inputPath);
	handbrakeCLI.spawn(options.handbrake, options.handbraker.v || options.handbraker.verbose);
};
