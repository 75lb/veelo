...
Job.prototype.process = function(){
    ...
    options.handbrake.i = this.path.input;
    options.handbrake.o = this.path.working;
        ...
        if (fs.existsSync(self.path.working)){
            ...
            fse.copy(self.path.working, self.path.output, function(err){
                ...
                if(options.veelo["preserve-dates"]){
                    var inputFileStats = fs.statSync(options.veelo.archive ? self.path.archive : self.path.input);
    ...
    handbrakeCLI.spawn({
        handbrakeArgs: options.handbrake, 
        emitOutput: options.veelo.verbose
    });
};
