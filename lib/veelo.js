...
exports.encode = function(files, options, handbrakeOptions){
    // validate options and handbrakeOptions
    var queue = new Queue();
    ...
    files.forEach(function(file){
        ...
        job.command = encodeCommand(file, workingPath, handbrakeOptions, { 
            "embed-srt": options["embed-srt"], 
        ...
        if (options.archive){
            var from = file,
                to = general.getSubDirPath(file, options.archiveDirectory),
        ...
        if (options["output-dir"]){
            ...
            general.moveFile(workingPath, outputPath);
        }
    });
