...
Veelo.prototype.start = function(){
    ...
    // user passes options that don't require files
        ...
        handbrakeCLI.exec({ handbrakeArgs: config.toJSON("handbrake") }, function(stdout, stderr){
...
Veelo.configDefinition = [
    {
        ...
        options: [
            ...
            { name: "archiveDirectory", type: "string" },
...
function initConfig(){
    config.group("veelo")
        ...
        .option("ext", { type: "string", valid: "\.mp4|\.m4v|\.mkv" })
        .option("archive", { type: "boolean" })
        .option("archiveDirectory", { type: "string" })
    ...
    config.set("ext", "m4v")
          .set("ignoreList", [])
          .set("archiveDirectory", "veelo-originals");
