            ...
            files.forEach(function(file){
                ...
                if (fs.statSync(file).isFile()){
                    var job = new Job("Encode file: " + file),
                        newFile = general.replaceFileExtension(file, config.get("ext")),
                        workingPath = general.getTempFilePath(newFile);
                        
                    job.command = encodeCommand(
                        file, 
                        workingPath, 
                        config.group("handbrake").toJSON(),
                    ...
                    if (config.get("output-dir")){
                        var outputDir = config.get("output-dir").trim(),
                            outputPath = "";
...
function encodeCommand(inputFile, outputFile, handbrakeOptions, embedSrt, verbose){
    function command(){
        ...
        handbrake.run(handbrakeOptions)
            ...
            .on("output", function(data){ verbose && commandHandle.emit("info", data); })
...
configs.add(
    "help",
    new Config()
        .option("topic", { type: "string", valid: /^(encode|info|handbrake)$/, default: "encode", defaultOption: true })
);
exports.help = function(options, done){
    var config = configs.get("help", options);
