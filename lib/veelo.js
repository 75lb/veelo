...
configs.add(
    ...
    new Config()
        ...
        .define({ name: "archive-directory", type: "string", default: "veelo-originals", valid: function(dir){
            var sameAsOutputDir = path.resolve(dir) === path.resolve(this.config.get("output-dir"));
            if (sameAsOutputDir){
                this.addValidationError("Archive and Output directories must be different");
            }
            return !sameAsOutputDir;
        }})
