...
Veelo.prototype.config = config;

Veelo.prototype.add = function add(file){
    config.get("inputFiles").push(file);
    this.queue.add(file);
    return this;
};
...
Veelo.prototype.start = function(){
    ...
    // config
        ...
        self.emit("message", util.inspect(config._getActive(), false, null, true));
...
Veelo.prototype.onHandbrakeOutput = function onHandbrakeOutput(callback){
    this.queue.on("handbrake-output", callback);
};
Veelo.prototype.onMessage = function onMessage(callback){
...
function initConfig(){
    ...
    config
        ...
        .parseCliArgs({
            ...
            onDone: function(){
                self.queue.add(config.get("inputFiles"));
            }
        });
}