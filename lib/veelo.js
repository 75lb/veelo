...
exports.encode = function(files, options, handbrakeOptions){
    var queue = new Queue();
    var encodeConfig = new JobOptions(options);
    
    files.forEach(function(file){
        var job = new Job("Encode file: " + file);
        job.command = encodeCommand(file);
        queue.add(job);
...
function encodeCommand(file, handbrakeOptions, embedSrt, workingPath, verbose){
    function command(){
        var commandHandle = new EventEmitter();
            
        // embed subtitles
        var srtPath = file.replace(/\.\w+$/, "." + "srt");
        if (jobOptions.get("embed-srt") && fs.existsSync(srtPath)){
            encoderOptions.set("srt-file", srtPath);
            this.emit(e.msg, "embedding external subs: " + srtPath);
        }
        
        _encoderConfig.i = file;
        _encoderConfig.o = this.path.working;
    
        handbrake.run(_encoderConfig)
            .on("starting", function(stats){
                commandHandle.emit("starting", stats);
            })
            .on("progress", function(progress){
                commandHandle.emit("progress", progress);
            })
            .on("output", function(data){
                _jobConfig.get("verbose") && commandHandle.emit("info", data);
            })
            .on("error", function(msg, options, output){
                commandHandle.emit(e.error, msg, options, output);
            })
            .on("terminated", function(){
                commandHandle.emit(e.info, "shutting down..");
                if (fs.existsSync(self.path.working)){
                    commandHandle.emit(e.info, "cleaning up");
                    fs.unlink(self.path.working);
                }
                commandHandle.emit(e.complete);
            })
