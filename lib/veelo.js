...
// this module is a Singleton event emitter
...
veelo.onHandbrakeOutput = function onHandbrakeOutput(callback){
    veelo.queue.on("handbrake-output", callback);
};
veelo.onMessage = function onMessage(callback){
    this.on("message", callback);
    veelo.queue.on("message", callback);
};
...
config
    ...
    .parseCliArgs({
        onInvalidArgs: function(invalid){
            veelo.emit("warning", "Invalid options: " + invalid.join(", "));
        },
        onDone: function(){
            veelo.queue.add(config.get("inputFiles"));
        }
    });
